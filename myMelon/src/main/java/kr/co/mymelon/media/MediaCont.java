...
public class MediaCont {
	...
	@RequestMapping(value="media/list.do")
	public ModelAndView list(MediaDTO dto) {
	    ...
	    mav.addObject("mediagroupno", dto.getMediagroupno());//부모글 번호
	    return mav;
	}//list() end
	...
	public ModelAndView createProc(@ModelAttribute MediaDTO dto //spring에서는 파일첨부시 request써서 받아오면 null값나옴 dto사용하세요
...
//----------------------------------------------------------
		...
		//1)<input type='file' name='filenameMF'>파일 가져오기
...
//----------------------------------------------------------
		...
		if(cnt==0) {
			...
			mav.addObject("img","<img src='../images/speaker.png'>");
			mav.addObject("link1","<input type='button' value='그룹목록' onclick='location.href=\"./list.do?mediagroupno= " + dto.getMediagroupno() +"\"'>");
		}//end
		return mav;
	}//createProc()end
	...
	//-----------------------------------------------------------
	    ...
	    //1)
	    MultipartFile posterMF=dto.getPosterMF();
	    if(posterMF.getSize()>0) {//새로운 포스터파일이 전송되었는지?
	    	//기존 파일 삭제
	  ...
	  //2)
	    MultipartFile filenameMF=dto.getFilenameMF();
	    if(filenameMF.getSize()>0) {//새로운 포스터파일이 전송되었는지?
	...
	//-----------------------------------------------------------
		...
		if(cnt==0) {
			mav.addObject("msg1", "<p>음원 파일 수정 실패</p>");
			mav.addObject("img", "<img src='../images/fail.png'>");
			mav.addObject("link1", "<input type='button' value='다시시도' onclick='javascript:history.back()'>");
			mav.addObject("link2", "<input type='button' value='목록으로' onclick='location.href=\"./list.do?mediagroupno=" + dto.getMediagroupno()+"\"'>");
		}else {
			mav.addObject("msg1", "<p>음원 파일 수정 성공</p>");
			mav.addObject("img","<img src='../images/speaker.png'>");
			mav.addObject("link1","<input type='button' value='그룹목록' onclick='location.href=\"./list.do?mediagroupno= " + dto.getMediagroupno() +"\"'>");
		}//end
